...
Covers ``JW.agent._ensure_auxiliary_chat_model`` (fallback to
...
class TestAuxiliaryResolver:
    ...
    def test_set_builds_auxiliary(self, monkeypatch):
        ...
        monkeypatch.setattr("jw.llm.get_chat_model", get_chat_model)
...
class TestAuxiliaryMiddlewareScope:
    ...
    def test_main_agent_tool_selector_aux_context_editing_main(self):
        ...
        with (
            ...
            patch(
                "jw.middleware.create_context_editing_middleware",
                side_effect=fake_ce,
            ),
        ):
