...
class TestMCPCommandDispatch:
    async def test_no_args_lists(self):
        ...
        ctx, ui = _ctx()
        with patch("jw.mcp.load_mcp_config", return_value={}):
    ...
    async def test_list_subcommand(self):
        ...
        with patch("jw.mcp.load_mcp_config", return_value=cfg):
    ...
    async def test_add_subcommand_dispatches(self):
        ...
        with (
            patch(
                "jw.mcp.parse_mcp_add_args",
                return_value={"name": "srv1"},
            ),
            patch(
                "jw.mcp.add_mcp_server",
    ...
    async def test_edit_subcommand_dispatches(self):
        ...
        with (
            patch(
                "jw.mcp.parse_mcp_edit_args",
                return_value=("srv1", {"tools": ["bar"]}),
            ),
            patch(
                "jw.mcp.edit_mcp_server",
            ) as edit_mock,
        ):
    ...
    async def test_remove_subcommand_success(self):
        ...
        ctx, ui = _ctx()
        with patch("jw.mcp.remove_mcp_server", return_value=True):
    ...
    async def test_remove_subcommand_not_found(self):
        ...
        ctx, ui = _ctx()
        with patch("jw.mcp.remove_mcp_server", return_value=False):
