...
WORKSPACE_RUN = ROOT / ".morphology-workspace-v24-20260827/projects/default/runs/run_01a0414e-700d-75c3_28ecad54"
EVAL_RUN = ROOT / "research/review/evals/runs/main_cycle_morphology.v24"
OUTPUT_DOCX = ROOT / "docs/SILSO太阳活动周形态实验-P5-P6评委展示稿.docx"
OUTPUT_MD = ROOT / "docs/SILSO太阳活动周形态实验-P5-P6评委展示稿.md"


def _stats_rows(stats: dict) -> list[list[str]]:
    judgments = {"length": "对照关系", "rise": "H1 主证据", "decline": "对照：方向不稳定"}
    labels = {"length": "周期长度—峰值", "rise": "上升时间—峰值", "decline": "下降时间—峰值"}
    rows: list[list[str]] = []
...
def _write_markdown(stats: dict) -> None:
    ...
    lines = [
        ...
        "|---|---|---|---|---|",
        "| H1 | 在统一官方边界与峰值定义下，上升时间越短，活动周峰值通常越高。 | Pearson、Spearman 同向为负；bootstrap 区间均低于 0；逐周期留一和固定分期不改方向。 | **高（历史描述性）**；证据链完整，但不外推为因果或未来预测。 | 中 |",
    ...
    lines += [
        ...
        "",
        "评委判读建议：H1 回答“历史形态是否反复出现”；H2 回答“前兆是否可能带来预测增益”；H3 回答“观测量是否包含额外信息”。三条结论回答的问题不同，不能合并成一个总置信度。人工证伪审查确认它们可检验，但不替代各自的实证支持。",
        ...
        "",
        "本案例从原始研究问题出发，沿“数据核验—假设生成—实验设计—结果复核—发布审查”链条完成闭环。最终没有把三条假设压成同一证据等级：H1 为限域高置信历史关系，H2 为中低置信方向性增益信号，H3 为数据不足、暂不可评级。",
        "",
        ...
        "",
        "### 6.2 已修复的经典失败：把可证伪性误写成同等级实证支持",
        "",
        "早期整理容易把人工物理论证、历史相关、样本外回测和数据准备状态并列为“三条都已验证”。修正后，人工审查只回答假设是否物理自洽、能否被证伪；支持度分别由各自证据决定。评委可以同时看到 H1 的稳定历史关系、H2 的方向性增益信号，以及 H3 的数据待补齐状态，结论更清晰也更可信。",
    ]
...
def build_docx(stats: dict) -> None:
    ...
    doc.sections[0].header.paragraphs[0].text = "太阳物理假设生成与证据推理｜WDC–SILSO 官方数据"
    add_banner(doc, "P5｜科学输出（核心章节）", "三个优先科学假设：WDC–SILSO Version 2.0 数据与分层证据")
    add_callout(
        doc,
        "结论边界",
    ...
    add_table(
        ...
        [
            ["H1", "统一官方边界与峰值定义下，上升时间越短，活动周峰值通常越高。", "Pearson、Spearman 同向为负；bootstrap 区间均低于 0；留一和固定分期不改方向。", "高（历史描述性）；证据链完整，但不外推为因果或未来预测。", "中"],
            ["H2", "极小期大尺度极区场能够改善下一活动周峰值预测。", "严格时序回测中候选 MAE 低于两类基线；新增留出折需继续验证改善是否稳定。", "中低（方向性增益信号）；点估计改善 13.053，但 95% 区间仍跨 0。", "高"],
            ["H3", "轴向偶极矩比普通极区孔径场提供额外预测信息。", "同一留出协议下，轴向偶极矩相对孔径场稳定降低误差并通过敏感性检查。", "暂不可评级（研究价值高）；物理构造已区分，实证输入待补齐。", "高"],
        ],
        widths=[1.0, 5.0, 5.3, 5.1, 1.3],
    )
    doc.add_heading("5.2 证据链构建", level=2)
    add_callout(doc, "证据链", "WDC–SILSO Version 2.0 官方数据 → H1 历史形态统计 → H2 极区场时间顺序回测 → H3 轴向偶极矩数据核对 → 人工证伪审查 → 分层结论。", LIGHT_TEAL)
    add_bullets(doc, [
    ...
    add_table(doc, ["关系", "Pearson r（双侧 p）", "Pearson 95% CI", "Spearman ρ（双侧 p）", "Spearman 95% CI", "判断"], _stats_rows(stats), widths=[3.0, 2.8, 3.1, 2.8, 3.1, 2.4])
    add_callout(doc, "H2 回测摘要", "5 个严格时序留出折：候选 MAE 26.972，优于均值 40.026 和持续性 64.231；改善 13.053，但 95% 区间 [-6.999, 31.299] 跨 0。结论是“有方向性增益信号”，尚非稳定预测技能；H3 输入待补齐。", LIGHT_BLUE)
    add_picture(doc, WORKSPACE_RUN / "outputs/cycle_morphology_relationships.png", 7.0, "图 1｜H1 的 WDC–SILSO Version 2.0 形态关系核验；H2/H3 使用独立观测量与实验协议。")

    doc.add_heading("5.3 置信度分层", level=2)
    add_table(doc, ["结论", "置信度", "依据摘要"], [
        ["H1：上升时间—峰值历史负相关", "高（限域）", "24 个完整周期中 Pearson r=−0.7495、Spearman ρ=−0.7619；两类 bootstrap 95% 区间均低于 0，留一及分期方向一致。支持稳定历史关系，不支持因果或样本外技能。"],
        ["H2：极区场可能带来样本外预测增益", "中低（方向性信号）", "5 个严格时序留出折中，MAE 26.972 对比 40.026/64.231，点估计改善 13.053；但 95% 区间 [-6.999, 31.299] 跨 0，MWO 仅 1 个测试折。有增益线索，尚未跨过稳定技能门。"],
        ["H3：轴向偶极矩具有额外预测信息", "暂不可评级（数据待补齐）", "轴向偶极矩与孔径场的构造和物理含义已区分；尚无可复算的全日面磁图及固定球谐产品，不能把“尚不可评估”写成“未发现增益”。"],
    ], widths=[5.0, 2.4, 9.9])
    add_callout(doc, "人工证伪审查说明", "审查确认三条假设的变量、机制方向、时间顺序和撤回条件彼此自洽；它回答“能否被检验”，不替代历史统计、样本外回测和数据登记各自回答的实证问题。三条结论不能合并成一个总置信度。", LIGHT_BLUE)

    doc.add_heading("5.4 反例与不支持证据", level=2)
    add_table(doc, ["反例或限制", "相关假设", "来源", "对假设的影响"], [
        ["H1 的负导数推导依赖扩散占优条件；历史相关不能单独证明条件成立。", "H1", "人工证伪审查", "保留高置信历史表述，暂不升级为因果机制。"],
        ["H1 仅有 24 个完整活动周，测量制度和周期依赖可能影响相关强度。", "H1", "官方数据统计", "“高”限于当前样本、定义和历史描述任务。"],
        ["H2 改善区间跨 0，5 折中有 1 折为负；MWO/WSO 制度不均衡。", "H2", "时间顺序回测", "保留方向性增益，暂不授予稳定预测技能。"],
        ["尚无轴向偶极矩产品或可复算全日面磁图。", "H3", "数据登记核对", "保留研究价值；状态为“暂不可评估”，不是“假设失败”。"],
    ...
    add_table(doc, ["假设", "验证方法", "所需数据/设施", "周期", "成功判据"], [
        ["H1", "按同一官方定义接入新增完整活动周后重算相关、留一和分时期结果。", "WDC–SILSO Version 2.0 后续官方极值与平滑序列", "新完整周期形成后", "负方向和区间保持；否则降低或撤回限域支持。"],
        ["H2", "锁定当前模型，扩大时间顺序留出窗口并分测量制度复核。", "同口径 MWO/WSO 极区场与后续官方峰值", "新目标周期完成后", "多个新增留出折持续优于基线，改善区间不跨 0。"],
        ["H3", "注册全日面磁图并固定球谐定义，与孔径场采用同一滚动留出协议。", "可复算磁图、覆盖说明和缺测规则", "数据齐备后", "独立留出周上稳定降低误差并通过敏感性检查。"],
    ], widths=[1.0, 5.4, 4.1, 2.1, 4.0])

    add_banner(doc, "P6｜典型案例展示", "一条完整研究闭环 + 一个已修复的科学表述失效模式")
    doc.add_heading("6.1 成功案例：从原始问题到可复核发布结果", level=2)
    add_callout(doc, "正式运行结果", "本案例从原始研究问题出发，沿“数据核验—假设生成—实验设计—结果复核—发布审查”链条完成闭环。最终没有把三条假设压成同一证据等级：H1 为限域高置信历史关系，H2 为中低置信方向性增益信号，H3 为数据不足、暂不可评级。")
    add_bullets(doc, [
    ...
    add_picture(doc, EVAL_RUN / "screenshot.png", 6.2, "图 2｜一次完整正式运行的结果页；对应数据、计算结果与复核记录均已归档。")

    # Keep the wide failure-analysis table together instead of leaving a
    # header/partial row at the bottom of the preceding page.
    heading = doc.add_heading("6.2 已修复的经典失败：把可证伪性误写成同等级实证支持", level=2)
    heading.paragraph_format.page_break_before = True
    add_table(doc, ["观察", "根因", "修复", "修复后证明"], [[
    ...
    add_callout(doc, "评委可核对的交付标准", "研究阶段完成不等于结论成立。最终交付必须给出清晰的结论边界、可复算的关键数字和独立的证据复核。", LIGHT_GOLD)
...
def main() -> None:
    stats = compute_statistics(_read_cycle_rows(WORKSPACE_RUN))
