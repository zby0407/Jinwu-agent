...
def bootstrap_difference(
    ...
    return float(np.mean(baseline_error) - np.mean(candidate_error)), percentile_ci(
        draws
    )


def build_cycle_table(
    monthly: pd.DataFrame, smoothed: pd.DataFrame, extrema_path: Path
) -> pd.DataFrame:
    ...
    for cycle in range(1, 25):
        ...
        if any(d not in raw.index for d in dates):
            raise ValueError(
                f"raw monthly input missing early-rise months for cycle {cycle}"
            )
        rows.append(
            {
                "cycle": cycle,
                ...
                "rise_slope": slope(
                    offsets.astype(float), raw.loc[dates].to_numpy(float)
                ),
                "rise_months": int(
                    month_offset(record.minimum.date, record.maximum.date)
                ),
            }
        )
    ...
    c25_peak_window = smoothed[
        (smoothed["date"] >= c25.minimum.date)
        & (smoothed["date"] <= smoothed["date"].max())
    ]
    ...
    rows.append(
        {
            ...
            "rise_slope": slope(
                c25_offsets.astype(float), raw.loc[c25_dates].to_numpy(float)
            ),
            "rise_months": int(
                month_offset(c25.minimum.date, pd.Timestamp(c25_peak_row["date"]))
            ),
        }
    )
...
def same_cycle_backtest(
    cycles: pd.DataFrame, rng: np.random.Generator
    ...
    for idx in range(train_end, 24):
        ...
        pred = fit_predict(
            train["rise_slope"].to_numpy(float),
            train["peak"].to_numpy(float),
            float(test["rise_slope"]),
        )
        baseline = float(train["peak"].mean())
        training_cycles = [int(value) for value in train["cycle"]]
        rows.append(
            {
                "cycle": int(test["cycle"]),
                "training_cycle_start": training_cycles[0],
                "training_cycle_end": training_cycles[-1],
                "observed_peak": float(test["peak"]),
    ...
    leakage_passed = bool((frame.training_cycle_end < frame.cycle).all())
    return frame, {
        "test_cycles": len(frame),
        "candidate_mae": float(ce.mean()),
        "baseline_mae": float(be.mean()),
        "mae_improvement": improvement,
        "mae_improvement_ci95": ci,
        "candidate_wins": int((ce < be).sum()),
        "leakage_audit_passed": leakage_passed,
    }


def next_cycle_backtest(
    cycles: pd.DataFrame, rng: np.random.Generator, model: str
) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    for target in range(10, 25):
        ...
        train_y = (
            cycles[cycles.cycle.isin(train_targets)]
            .sort_values("cycle")["peak"]
            .to_numpy(float)
        )
        ...
        training_cycles = [int(value) for value in train_targets]
        rows.append(
            {
                "cycle": target,
                "training_cycle_start": training_cycles[0],
    ...
    return frame, {
        ...
        "candidate_rmse": float(
            np.sqrt(np.mean((frame.observed_peak - frame.candidate_prediction) ** 2))
        ),
        "baseline_rmse": float(
            np.sqrt(np.mean((frame.observed_peak - frame.baseline_prediction) ** 2))
        ),
        "mae_improvement": improvement,
        "mae_improvement_ci95": ci,
        "candidate_wins": int((ce < be).sum()),
...
def formal_forecast(
    ...
    pred_both = fit_predict_multi(
        np.column_stack([x_peak, x_rise]),
        y,
        np.array([prev["peak"], prev["rise_slope"]]),
    )
    ...
    for i in range(BOOTSTRAP_REPS):
        ...
        draws[i] = max(
            0.0, float(np.polyval(beta, prev["peak"]) + rng.choice(residual[idx]))
        )
    return {
        "target": "Cycle 26 peak 13-month-smoothed SILSO v2 sunspot number",
        "as_of": "2026-08-27",
        "primary_model": "expanding-history linear regression: predecessor peak -> next-cycle peak",
        "cycle_25_peak_used": float(prev["peak"]),
        "cycle_25_rise_slope_used": float(prev["rise_slope"]),
        "point_estimate": pred_peak,
        "predictive_interval_95": percentile_ci(draws),
        "sensitivity_lag_peak_rise": pred_both,
        "climatology_mean_cycles_1_24": float(train.peak.mean()),
        "bootstrap_seed": SEED,
        "bootstrap_repetitions": BOOTSTRAP_REPS,
        "confidence": "low",
        "interpretation": "Historical backtests do not establish positive skill over the mean baseline; the interval is deliberately wide and the estimate is a conditional statistical forecast, not a physical certainty.",
    }
...
def make_figure(
    ...
    ax.plot(
        lag.cycle,
        lag.candidate_prediction,
        "s--",
        color=colors["candidate"],
        label="Lag-peak model",
    )
    ax.plot(
        lag.cycle,
        lag.baseline_prediction,
        ":",
    ...
    for _, r in lag.iterrows():
        ax.annotate(
            f"SC{int(r.cycle)}",
            (r.cycle, r.observed_peak),
            xytext=(0, 6),
            textcoords="offset points",
            fontsize=7,
            ha="center",
        )
    ax.set_title("Time-ordered backtest (SC10–24)")
    ax.set_xlabel("Target cycle")
    ax.set_ylabel("Peak (13-month smoothed SN)")
    ax.legend(fontsize=8)
    ax = axes[1]
    ax.scatter(
        same.candidate_prediction,
        same.observed_peak,
        color=colors["candidate"],
        label="Same-cycle rise model",
    )
    ax.scatter(
        lag.candidate_prediction,
        lag.observed_peak,
    ...
    ax.set_title("Predicted vs observed (closer to dashed line is better)")
    ax.set_xlabel("Predicted peak")
    ax.set_ylabel("Observed peak")
    ax.legend(fontsize=8)
    ax = axes[2]
    labels = ["Lag peak", "Lag peak + rise", "Historical mean"]
    vals = [
        float(forecast["point_estimate"]),
        float(forecast["sensitivity_lag_peak_rise"]),
        float(forecast["climatology_mean_cycles_1_24"]),
    ]
    ...
    ax.axhline(
        float(cycles[cycles.cycle == 25].peak.iloc[0]),
        color="#16a085",
        ls="--",
        label="SC25 official smoothed peak",
    )
    ax.set_title("Formal Cycle 26 forecast (as of 2026-08-27)")
    ax.set_ylabel("Peak (13-month smoothed SN)")
    ax.tick_params(axis="x", labelrotation=18)
    ax.legend(fontsize=8)
    fig.suptitle(
        "JW Solar Cycle Peak Strength: Backtest and Cycle 26 Forecast",
        fontsize=15,
        fontweight="bold",
    )
...
def write_report(
    out: Path,
    cycles: pd.DataFrame,
    same_stats: dict[str, object],
    lag_stats: dict[str, object],
    both_stats: dict[str, object],
    forecast: dict[str, object],
...
def main() -> None:
    ...
    args = ap.parse_args()
    monthly_path, smoothed_path, extrema_path = map(
        lambda x: Path(x).resolve(), (args.monthly, args.smoothed, args.extrema)
    )
    ...
    pd.concat(
        ...
        ignore_index=True,
    ).to_csv(out / "sc26_forecast_predictions.csv", index=False)
    (out / "sc26_formal_forecast.json").write_text(
        json.dumps(forecast, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    (out / "forecast_evidence_gate.json").write_text(
        json.dumps(evidence_gate, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    sources = {
        "retrieved": "2026-08-27",
        "monthly": {"path": str(monthly_path), "sha256": sha256(monthly_path)},
        "smoothed": {"path": str(smoothed_path), "sha256": sha256(smoothed_path)},
        "extrema": {"path": str(extrema_path), "sha256": sha256(extrema_path)},
    }
    (out / "data_manifest.json").write_text(
        json.dumps(sources, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    make_figure(
        cycles, same, lag, lag_both, forecast, out / "sc26_forecast_visualization.png"
    )
