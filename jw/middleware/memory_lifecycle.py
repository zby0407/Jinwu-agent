"""Middleware that schedules post-run JWMemory workers."""
...
class JWMemoryLifecycleMiddleware(AgentMiddleware):
    """Schedule post-turn and post-subagent memory workers."""

    name = "jwmemory_lifecycle"
    ...
    def after_agent(
        ...
        if context is not None:
            try:
                ...
                logger.warning("Failed to launch JWMemory worker", exc_info=True)
                self._memory_scheduler.flush_ready()
        else:
            self._memory_scheduler.flush_ready()
        return None
...
def create_memory_lifecycle_middleware(
    ...
    *,
    ...
    """Build the post-run JWMemory lifecycle middleware."""

    if memory_dir is None:
        memory_dir = str(_paths.MEMORIES_DIR)
    return JWMemoryLifecycleMiddleware(
