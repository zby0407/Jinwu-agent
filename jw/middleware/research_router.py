                    ...
                    if "solar-experiment" not in completed:
                        ...
                        if required_analysis_protocol == "f107_discontinuity_v1":
                            directive.append(
                                ...
                                "OLS slope. Compute the percentage jump at a fixed SN "
                                "reference (e.g., SN=100) and emit it under the "
                                "f107_relative_scale_jump measurement id. Compare it to "
                                "the published ~10.5% discontinuity. Treat the fixed "
                                "1980-1981 comparison as confirmatory. For an exploratory "
                                "breakpoint scan use an upper-tail survival probability "
                                "and select by maximum F (or minimum unrestricted SSR), "
                                "never the first underflowed p=0 candidate. Emit all "
                                "required measurement ids, define cross-period residuals "
                                "as actual-minus-predicted, and include observed/URSI, "
