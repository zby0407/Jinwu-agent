"""MCP server registry — marketplace index from JWSkills.

Provides MCP server definitions used by:
- ``/install-mcp`` (interactive browser and direct install)
- ``jw onboard`` (initial setup wizard, filters by ``onboarding`` tag)
- ``jw mcp install`` (CLI command)

Server definitions live in ``JWSkills/mcp/`` as individual YAML files.
"""
...
def _uv_tool_name() -> str | None:
    ...
    (e.g. ``/home/user/.local/share/uv/tools/jw`` → ``jw``).
    """
...
def pip_install_hint() -> str:
    ...
    if _is_uv_tool_env():
        return "uv tool install --reinstall jw --with"
    if shutil.which("uv"):
...
def _install_with_uv_tool_env(package: str) -> bool:
    """Install *package* into the jw uv-tool env via ``--with``.
    ...
    recorded in uv's receipt and survives ``uv tool upgrade jw``.
    """
...
def install_library(package: str) -> bool:
    ...
    1. **uv tool env**: ``uv tool install jw --with <pkg>`` —
       package lands in jw's isolated env and is importable from it.
...
def install_cli_tool(package: str, *, verify_command: str) -> bool:
    ...
    *verify_command* is the CLI name expected to appear after step 2; if
    ...
    # jw's own bin dir (next to sys.executable), not in `uv tool dir
...
# =============================================================================
# Marketplace index (YAML files in JWSkills/mcp/)
# =============================================================================
...
def fetch_marketplace_index(
    repo: str = "zby0407/JW-Skills",
    ref: str | None = None,
    path: str = "mcp",
) -> list[MCPServerEntry]:
    """Fetch MCP server definitions from the marketplace.

    Shallow-clones the JWSkills repo and scans ``{path}/*.yaml``.
    ...
    entries: list[MCPServerEntry] = []
    with tempfile.TemporaryDirectory(prefix="jw-mcp-browse-") as tmp:
    ...
    if entry.env_key:
        if entry.env_optional:
            ...
            if not os.environ.get(entry.env_key):
                print_fn(
                    f"  Set it before running JW: export {entry.env_key}=...",
                    "dim",
                )
