...
> Connects external systems to [JW](https://github.com/zby0407/Jinwu-agent) via [Model Context Protocol (MCP)](https://modelcontextprotocol.io/).
...
```bash
jw mcp add <name> <command-or-url> [args...] \
  [--transport <transport>] \
  [--tools <tool1,tool2,...>] \
...
| Command | Description |
|---------|-------------|
| `jw mcp` | List configured servers |
| `jw mcp list` | List configured servers |
| `jw mcp config` | Show detailed config for all servers |
| `jw mcp config <name>` | Show detailed config for one server |
| `jw mcp add ...` | Add a server |
| `jw mcp edit ...` | Edit an existing server |
| `jw mcp remove <name>` | Remove a server |
...
jw mcp add research-sse https://example.com/sse --transport sse
...
jw mcp add fs npx -t "read_*,write_*" -- -y @modelcontextprotocol/server-filesystem /workspace

# Context7 routed to multiple agents
jw mcp add context7 npx -e main,research-agent,code-agent -- -y @upstash/context7-mcp@latest
```
...
jw mcp add filesystem npx -- -y @modelcontextprotocol/server-filesystem /tmp

# http transport (auto-detected from URL)
jw mcp add brave-search http://localhost:8080/mcp -H "Authorization:Bearer ${BRAVE_API_KEY}"

# sse transport, routed to a specific agent
jw mcp add my-sse http://localhost:9090/sse --transport sse -e research-agent

# With tool allowlist (supports glob wildcards)
jw mcp add fs npx -- -y @modelcontextprotocol/server-filesystem /tmp -t "read_*,write_*"
```
...
jw mcp edit filesystem --tools "read_*,write_*"

# Clear a tool allowlist (pass all tools)
jw mcp edit filesystem --tools none
...
> [!NOTE]
> Tools routed to sub-agents are injected automatically — no need to edit any `jw/subagents/*.yaml` file.
...
> Path: `~/.config/jw/mcp.yaml` (or `$XDG_CONFIG_HOME/jw/mcp.yaml`)
...
1. On agent startup (`/new` or `create_cli_agent()`), reads `~/.config/jw/mcp.yaml`
...
Auth header/env becomes empty — ensure the variable exists in your environment before launching JW. `${VAR}` interpolation happens at runtime; missing vars are replaced with empty strings and logged as warnings.
...
- Confirm endpoint/command is healthy outside JW
