...
def _patch_mcp_windows_command_resolver() -> None:
    ...
    if getattr(original, "_jw_absolute_fast_path", False):
    ...
    _patched_get_windows_executable_command._jw_absolute_fast_path = True  # type: ignore[attr-defined]
...
def load_mcp_config() -> dict[str, Any]:
    ...
    Reads ``~/.config/jw/mcp.yaml`` and interpolates ``${VAR}``
...
def _route_tools(
    ...
    Returns:
        ...
        main JW agent; other keys match subagent names.
    """
...
def load_mcp_tools(
    ...
    *,
    ...
    1. Loads user config from ``~/.config/jw/mcp.yaml``
    ...
    Args:
        config: Optional pre-loaded MCP config dict.  When ``None``,
            loads from ``~/.config/jw/mcp.yaml``.  Passing a
