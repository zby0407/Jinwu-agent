...
that hosts the JW main agent, async sub-agents (e.g.
``writing-agent``), and JWMemory background workers. The CLI calls
``ensure_langgraph_dev(config, ...)`` at startup so users can run
``jw -p "..."`` without manually managing the langgraph dev server.
...
class LanggraphRuntimePaths:
    ...
    ``~/.config/jw``, so the function under test still wrote
...
# ``jw deploy`` is already running) read this on the reuse path to refuse
...
# sub-agents' cwd / JW_WORKSPACE_DIR env match the new workspace.
...
# - langgraph dev subprocess spawned by ``jw deploy``: starts True via
#   ``JW_DEPLOY_MODE=full`` env var. The deployed main agent IS the
#   langgraph dev server, so http://localhost:{port} is always reachable for
#   self-loop async sub-agent dispatch.
# - langgraph dev subprocess spawned by ``jw`` / ``jw serve``: env
...
_ASYNC_SUBAGENTS_AVAILABLE: bool = (
    os.environ.get("JW_DEPLOY_MODE", "").lower() == "full"
)
...
def _packaged_langgraph_config() -> Path:
    ...
    Lives at ``jw/langgraph_dev/langgraph.json`` and is included
    in the wheel via ``pyproject.toml`` ``package-data`` so it's available
    regardless of how JW was installed (pip / editable / source).
    """
    import jw.langgraph_dev as _pkg
    ...
    *,
    ...
    Args:
        ...
        deploy_mode: When True, the subprocess loads full MCP + async
            sub-agents (``JW_DEPLOY_MODE=full``); otherwise stripped.
    ...
    if exe is None:
        raise FileNotFoundError(
            "langgraph CLI not found. Reinstall JW (langgraph-cli is "
    ...
    if not config_file.exists():
        raise FileNotFoundError(
            ...
            "This indicates a broken JW installation — reinstall."
        )
    ...
    if not _wait_for_port_bindable(port):
        raise RuntimeError(
            ...
            f"or change ports with: `jw config set langgraph_dev_port <other-port>`"
        )
    ...
    #
    # Note: ``JW_WORKSPACE_DIR`` serves a dual role in this codebase.
    ...
    sub_env["JW_WORKSPACE_DIR"] = str(workspace_dir)
    ...
    #   jw config set langgraph_dev_file_persistence false
    ...
    #
    #   - ``JW_DEPLOY_MODE=full`` (deploy_mode=True): set by
    #     ``jw deploy``. Subprocess is the primary programmatic entry
    #     point; main agent loads MCP and ``_ASYNC_SUBAGENTS_AVAILABLE``
    #     flips to True at module load, enabling self-loop async dispatch.
    #
    #   - ``JW_DEPLOY_MODE=stripped`` (deploy_mode=False): set by
    #     ``jw`` / ``jw serve``. The CLI's main agent already loaded
    ...
    #
    #   - (unset): parent process or plain ``import jw``. Loads
    ...
    sub_env.pop("JW_DEPLOY_MODE", None)
    sub_env["JW_DEPLOY_MODE"] = "full" if deploy_mode else "stripped"
...
def ensure_langgraph_dev(
    ...
    still chat with sync sub-agents; only async sub-agent calls and JWMemory
...
def _ensure_langgraph_dev_locked(
    ...
    # different workspace), the deployed sub-agents' cwd / JW_WORKSPACE_DIR
    ...
    if is_langgraph_dev_running(port=port):
        ...
        # — we never owned it (jw deploy in another terminal, or a
    ...
    try:
        ...
        # calls at a dead URL. JWMemory workers will also skip until the
        # server is reachable.
        _ASYNC_SUBAGENTS_AVAILABLE = False
        logger.warning(
            "Failed to start langgraph dev — async sub-agents will fall back "
            "to in-process delegation, and JWMemory background workers will "
            "not run. %s",
            exc,
        )
        return None
