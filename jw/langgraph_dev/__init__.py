...
Holds everything needed to run the JW agent ecosystem on a local
...
- ``main_graph`` — re-export of the lazy-loaded ``jw_agent``.
- ``graphs`` — module-level bindings for every yaml-flagged async sub-agent
  (``async: true`` in ``jw/subagents/<name>.yaml``).

The graphs themselves are built by ``JW.subagents._factory.
