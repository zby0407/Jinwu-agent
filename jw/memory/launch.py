"""JWMemory LangGraph launch adapter."""
...
SUBAGENT_MEMORY_WORKER_GRAPH_ID = "jwmemory-subagent-worker"
TURN_MEMORY_WORKER_GRAPH_ID = "jwmemory-turn-worker"
OBSERVATION_LINKER_GRAPH_ID = "jwmemory-observation-linker"
...
def _runs_create_kwargs(payload: BackgroundRunPayload) -> BackgroundRunPayload:
    try:
        from jw.llm.patches import _merge_runs_config_kwargs
    except Exception:
        return payload
...
def _memory_worker_metadata(context: MemorySourceContext) -> dict[str, str]:
    return {
        "run_kind": f"jwmemory_{context.source_type.value}_worker",
    ...
    *,
    ...
    payload: BackgroundRunPayload = {
        ...
        "config": {
            "configurable": {
                "thread_id": thread_id,
                "jwmemory_source_session_id": context.session_id,
                "jwmemory_source_agent": context.source_agent,
                "jwmemory_project_id": context.project_id,
                "jwmemory_trajectory_digest": context.trajectory_digest,
            }
        },
    }
...
def _observation_linker_metadata(
    ...
    return {
        "run_kind": "jwmemory_observation_linker",
        "project_id": context.project_id,
    ...
    payload: BackgroundRunPayload = {
        ...
        "config": {
            "configurable": {
                "thread_id": thread_id,
                "jwmemory_project_id": context.project_id,
                "jwmemory_observation_ids": json.dumps(
...
def launch_memory_worker(
    ...
    *,
    ...
    """Launch one synchronous JWMemory worker for a source context."""
...
async def alaunch_memory_worker(
    ...
    *,
    ...
    """Launch one asynchronous JWMemory worker for a source context."""
