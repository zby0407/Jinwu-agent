...
JW provides unified integration with 10 messaging platforms. This document covers the architecture overview, message processing pipeline, capability matrix, security model, deployment guides, and troubleshooting.

Configuration file: `~/.config/jw/config.yaml` (or use environment variables with the `JW_` prefix).
...
jw config set channel_enabled telegram
jw config set telegram_bot_token "123456:ABC-xxx"

# Option C: Environment variables (JW_ prefix, uppercase)
export JW_CHANNEL_ENABLED=telegram
export JW_TELEGRAM_BOT_TOKEN="123456:ABC-xxx"
```

### 3. Start

```bash
jw serve                # Start agent + all enabled channels
# or
jw channel start        # Standalone channel mode (message loop only)
```
...
**Install:** `pip install jw[telegram]`
...
**Env vars:** `JW_TELEGRAM_BOT_TOKEN`, `JW_TELEGRAM_ALLOWED_SENDERS`, `JW_TELEGRAM_PROXY`
...
**Install:** `pip install jw[discord]`
...
**Env vars:** `JW_DISCORD_BOT_TOKEN`, `JW_DISCORD_ALLOWED_SENDERS`, `JW_DISCORD_ALLOWED_CHANNELS`, `JW_DISCORD_PROXY`
...
**Env vars:** `JW_SLACK_BOT_TOKEN`, `JW_SLACK_APP_TOKEN`, `JW_SLACK_ALLOWED_SENDERS`, `JW_SLACK_ALLOWED_CHANNELS`, `JW_SLACK_PROXY`
...
```bash
# Install the SDK
pip install 'jw[feishu]'

# Via config file
feishu_subscription_mode: "websocket"

# Via CLI (standalone)
python -m jw.channels.feishu.serve --app-id ID --app-secret SECRET --mode websocket
```
...
**Env vars:** `JW_FEISHU_APP_ID`, `JW_FEISHU_APP_SECRET`, `JW_FEISHU_WEBHOOK_PORT`, `JW_FEISHU_DOMAIN`
...
**Install:** `pip install jw[dingtalk]`
...
**Env vars:** `JW_DINGTALK_CLIENT_ID`, `JW_DINGTALK_CLIENT_SECRET`
...
**Env vars:** `JW_QQ_APP_ID`, `JW_QQ_APP_SECRET`

**Technical details:** Uses `qq-botpy` SDK via WebSocket to connect to QQ Bot Gateway. Supports C2C (direct) and group messages. Outbound replies prefer native QQ markdown messages (`msg_type=2`) so headings/lists/code fences keep their structure; when the SDK/API rejects markdown, JW falls back to plain text with Markdown stripped but line structure preserved. Message deduplication (1000-entry LRU cache). Group @mention filtering (strips first `@bot`). Intents: `public_messages=True`, `direct_message=True`. Text chunk limit: 4096 chars.

---

### Signal

**Install:** `pip install jw[signal]` (also requires [signal-cli](https://github.com/AsamK/signal-cli) installed separately)
...
2. Register or link a phone number:
   ...
   - Link existing device: `signal-cli link -n "JW"`
3. JW will auto-start the signal-cli daemon if it's not already running.
...
**Env vars:** `JW_SIGNAL_PHONE_NUMBER`, `JW_SIGNAL_CLI_PATH`, `JW_SIGNAL_RPC_PORT`
...
**Install:** `pip install jw[email]` (core dependencies included, no extras needed)
...
**Env vars:** `JW_EMAIL_IMAP_HOST`, `JW_EMAIL_IMAP_USERNAME`, `JW_EMAIL_IMAP_PASSWORD`, `JW_EMAIL_SMTP_HOST`, `JW_EMAIL_SMTP_USERNAME`, `JW_EMAIL_SMTP_PASSWORD`
...
**Env vars:** `JW_IMESSAGE_CLI_PATH`, `JW_IMESSAGE_SERVICE`, `JW_IMESSAGE_ALLOWED_SENDERS`
...
WORKDIR /app
COPY . .
RUN pip install jw[feishu,wechat]
...
docker run -d \
  -p 9000:9000 \
  -p 9001:9001 \
  -e JW_CHANNEL_ENABLED="feishu,wechat" \
  -e JW_FEISHU_APP_ID="cli_xxx" \
  -e JW_FEISHU_APP_SECRET="xxx" \
  -e JW_WECHAT_BACKEND="wecom" \
  -e JW_WECHAT_WECOM_CORP_ID="ww..." \
  -e JW_WECHAT_WECOM_SECRET="xxx" \
  jw
```
...
```bash
docker run -d \
  -e JW_CHANNEL_ENABLED="telegram" \
  -e JW_TELEGRAM_BOT_TOKEN="123456:ABC-xxx" \
  jw
```
...
2. Verify the bot token/credentials are correct: `jw config get telegram_bot_token`
...
```bash
pip install jw[telegram]  # or discord, slack, feishu, etc.
```
...
```bash
export JW_LOG_LEVEL=DEBUG
# or
jw config set log_level debug
```

Channel-specific log output is prefixed with the module path (e.g., `JW.channels.telegram.channel`).
