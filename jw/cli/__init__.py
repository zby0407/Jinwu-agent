...
``import jw.cli`` only costs what ``main()`` actually needs.  That
keeps ``jw --help`` fast — the heavy chat-model/TUI/langgraph imports
...
def main():
    ...
    config = load_config()
    _log_level = os.environ.get("JW_LOG_LEVEL", "") or config.log_level
    _configure_logging()
    app()
