        ...
        async with get_checkpointer() as checkpointer:
            ...
            async def _on_handle_session_resume(
                ...
                if workspace_dir:
                    ...
                    try:
                        ...
                        # Another jw process owns the langgraph dev
            ...
            async def _process_channel_message(msg: ChannelMessage) -> None:
                ...
                try:
                    ...
                    # Mirrors the TUI's behavior so ``/jwskills``, ``/mcp list``
                ...
                while state["running"]:
                    try:
                        ...
                        if (
                            ...
                            console.print(
                                "[dim]Run [bold]jw onboard[/bold] to set up your API key.[/dim]"
                            )
                            state["running"] = False
                            break
                        else:
...
def _wait_for_memory_workers_before_exit(
    *,
    ...
    def print_saved(observed: MemoryWorkerStatusSnapshot) -> None:
        ...
        if saved:
            console.print(f"[dim]JWMemory saved {', '.join(saved)}.[/dim]")

    def print_waiting(phase: MemoryActivityPhase) -> None:
        nonlocal announced
        if not announced:
            console.print(f"[dim]Waiting for JWMemory {phase}...[/dim]")
            announced = True

    def print_timeout(phase: MemoryActivityPhase) -> None:
        console.print(f"[dim]JWMemory {phase} is still running; shutting down.[/dim]")
