...
If `solar-hypothesis` fails or becomes unreachable, do not call the `scientific_hypothesis_*` tools from the main Agent and do not author substitute candidates or rankings. Report the specialist's real technical blocker and one safe recovery step; only a successful `solar-hypothesis` receipt counts as completion.

When a `solar-hypothesis` request references a bundled relative path under `hypothesis/inputs/`, pass the relative path unchanged in the `task` call even when generic workspace file tools cannot see it. The specialist's bind tool is the authorized resolver for that narrow input directory. The main Agent must not probe it with generic file tools; do not create, copy, move, or stage that input. If the user explicitly says to hand the task to `solar-hypothesis`, the first task-specific action is the real `task` call, not `read_file`, `glob`, `ls`, or `execute`.
