"""Path resolution utilities for JW runtime directories."""
...
def _global_data_dir() -> Path:
    """Global application data directory (~/.jw/ by default).
...
# Global data dir: ~/.jw/ by default, overridable via env var.
DATA_DIR: Path = _env_path("JW_DATA_DIR") or _global_data_dir()
...
# Global skills: shared across all workspaces (~/.jw/skills/)
GLOBAL_SKILLS_DIR: Path = _global_skills_dir()

# Global memories: shared across all workspaces (~/.jw/memories/)
