...
def _step_ui_backend(config: JWConfig) -> str:
...
def _step_langgraph_dev_port(config: JWConfig) -> int:
    """Step 0.5: Choose the local TCP port for the langgraph dev subprocess.

    jw auto-starts a ``langgraph dev`` server in the background to host
    ...
    if current_occupied and is_langgraph_dev_running(port=current_port):
        # Another jw shell is already serving on this port — reuse, don't
    ...
    def valid_port(value: str) -> bool:
        ...
        # occupier is our own langgraph dev (e.g., another jw shell) —
    ...
    # jw server on that port is fine. They can always change later via:
    # jw config set langgraph_dev_port <port>
    if _is_port_occupied(port) and not is_langgraph_dev_running(port=port):
        console.print(
            f"  [yellow]⚠ Port {port} is occupied. jw may fail to start its "
...
def _step_webui_port(config: JWConfig) -> int:
    ...
    the browser opens (``@jw/webui``); the backend keeps its own
...
def _step_minimax_region(config: JWConfig) -> str:
    ...
    *,
    ...
    if auth_mode == "oauth" and not ccproxy_available:
        ...
        install = questionary.confirm(
            'Install ccproxy now? (pip install "jw[oauth]")',
            default=True,
            style=WIZARD_STYLE,
            qmark=f"  {QMARK}",
        ).ask()
        ...
        if install:
            ...
            console.print(
                '  [dim]Skipped. Install manually: pip install "jw[oauth]"[/dim]'
            )
            return "api_key"
...
def _step_anthropic_auth_mode(config: JWConfig) -> str:
...
def _step_openai_auth_mode(config: JWConfig) -> str:
...
def _step_base_url(config: JWConfig, current_value: str | None = None) -> str:
...
def _step_ollama_base_url(config: JWConfig) -> tuple[str, list[str]]:
...
def _step_auxiliary_enable(config: JWConfig) -> bool:
    """Step 3.25: Choose whether to assemble a co-pilot (auxiliary) model.

    The co-pilot runs background/helper LLM calls — JWMemory (memory workers)
    ...
    """
    console.print(
        "  [dim]A cheaper/faster co-pilot for JWMemory (memory workers).[/dim]"
    )
...
def _step_reasoning_effort(config: JWConfig) -> str:
...
def _step_workspace(config: JWConfig) -> str:
...
def _step_thinking(config: JWConfig) -> bool:
...
_RECOMMENDED_SKILLS = [
    # ── Official (JW) ──
    {
        "label": "jw Skills  (optimized for JW — paper planning, writing, review, etc.) 👈 Recommended",
        "source": "zby0407/JW-Skills@skills",
    },
    # ── Third-party (K-Dense) ──
    {
...
def _step_skills() -> list[str]:
    ...
    if not selected:
        ...
        if has_npx:
            ...
            console.print(
                "  [yellow bold]* Less is more[/yellow bold] [dim](JW can discover and install skills on its own)[/dim]"
            )
        else:
...
def _step_mcp_servers() -> list[str]:
    ...
    try:
        ...
        console.print(
            ...
            "you can re-run with [bold]jw configure mcp[/bold] later.[/yellow]"
        )
        return []
    ...
    if not selected:
        ...
        console.print(
            "  [dim]Add later with: jw mcp add <name> <command> [--env-ref KEY] -- [args][/dim]"
        )
        return []
