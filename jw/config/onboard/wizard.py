...
def render_progress(current_step: int, completed: set[int]) -> Panel:
    ...
    content = Text("\n").join(lines)
    return Panel(content, title="[bold]JW Setup[/bold]", border_style="blue")
...
def _autosave(config: JWConfig) -> None:
...
def _active_llm_providers(config: JWConfig) -> set[str]:
...
def _reconcile_oauth_modes(config: JWConfig) -> None:
...
def _provider_uses_oauth(config: JWConfig, provider: str) -> bool:
...
def _provider_connection_configured(config: JWConfig, provider: str) -> bool:
...
def _config_has_meaningful_settings(config: JWConfig) -> bool:
    """True if the user has been through onboarding before.

    Compares ``config`` against fresh ``JWConfig()`` defaults — any
...
def _open_existing_config_prompt(
    config: JWConfig,
) -> tuple[frozenset[str], JWConfig] | None:
...
def run_onboard(
    ...
    Args:
        ...
        only_sections: If given, restrict the wizard to exactly these section
            ids — the Keep/Modify/Reset prompt is skipped. Used by ``jw
    ...
    to ``~/.config/jw/config.yaml`` immediately, so a Ctrl+C does
    ...
    try:
        with select_navigation_active():
            ...
            if only_sections is not None:
                ...
                console.print(
                    "[dim]  (Use 'jw configure <section>' or 'jw "
            ...
            if "port" in sections_to_run:
                ...
                if preset_port is not None:
                    ...
                    # ``JWConfig``) instead of opening the
            ...
            if "provider" in sections_to_run:
                ...
                _print_section("JW · Pilot (Main model)")
        ...
        console.print(
            f"[dim]  Run [bold]jw onboard[/bold] again to resume — "
