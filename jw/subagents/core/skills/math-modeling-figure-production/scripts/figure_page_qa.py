...
def pdf_pages(pdf: Path) -> int:
    ...
    out = subprocess.check_output(
        ["pdfinfo", str(pdf)], text=True, stderr=subprocess.STDOUT
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render PDF pages and flag likely figure pages for visual QA."
    )
    parser.add_argument("pdf", type=Path)
    parser.add_argument("--outdir", type=Path, default=Path("figure_page_qa"))
    parser.add_argument(
        "--pages", help="Comma/range page spec, e.g. 1,5-8. Defaults to all pages."
    )
    parser.add_argument("--dpi", type=int, default=150)
    parser.add_argument(
        "--only-figure-pages",
        action="store_true",
        help="Render only pages whose text contains figure captions.",
    )
    ...
    for page in pages:
        ...
        manifest.append(
            {"page": page, "image": str(image), "has_figure_caption": has_figure}
        )
