...
This skill combines research-backed writing with well-structured manuscripts and formal reports. For claims that require literature support, begin with `research-literature-first`; use `paper-lookup` for targeted discovery, `literature-review` for broad or systematic synthesis, and `citation-management` when references must be verified or managed. `research-lookup` is supplemental current web context, not the primary scholarly database.

Scientific writing communicates research with precision and clarity. The audience, decision purpose, venue, and evidence determine the structure; IMRAD is one option, not a universal report template.

**Critical Principle:** Write manuscript body text in full, connected paragraphs. Reports may use tables or functional lists when they improve retrieval or comparison. Use a two-stage process: first create section outlines tied to literature/project evidence, then convert prose sections into complete paragraphs.

## Choose the deliverable mode

Select one primary mode before outlining:

| Mode | Organizing question |
|---|---|
| Journal or conference manuscript | What contribution and evidence must satisfy the venue and field? |
| Research report | What question was studied, with what data and method, what was found, how should it be interpreted, and what remains uncertain? |
| Technical report | What task, system, or method was implemented, how was it verified, what results and operating limits were observed, and how can the work be reproduced? |
| Revision or reviewer response | Which requested changes affect language, evidence, method, mathematics, or scope, and where do they propagate? |

Use the venue or supplied template when it exists. Apply IMRAD only when it fits the study and audience. A report may use an executive summary, question, evidence base, method, findings, interpretation, limitations, and recommendations; a technical report may emphasize implementation and verification. Do not force every document into a journal-paper structure.

When revising, inspect the whole document and its evidence before changing a section. Keep substantive changes separate from language edits. Use `writing-reader-facing-content` as the final gate for visible prose, captions, appendices, and metadata.

## Literature-First Hard Gate

Do not draft or revise a serious scientific document from memory alone. Before writing substantive background, method rationale, novelty, comparison, discussion, or limitation claims, use `research-literature-first`. Choose `paper-lookup` for targeted sources or `literature-review` for a broad or systematic synthesis; add `citation-management` when the task needs verified metadata, a bibliography, or citation-style work. Use `research-lookup` only for supplemental current web context after scholarly sources are checked.

The literature-first pass must identify:

- landmark or foundational papers
- recent high-quality reviews, surveys, systematic reviews, or meta-analyses
- recent strong primary studies or state-of-the-art methods
- standard terminology, assumptions, methods, metrics, and reporting norms
- unresolved gaps, controversies, or limitations

If the user asks for a quick rough draft without literature lookup, label it explicitly as preliminary and do not present it as research-grade, submission-ready, or fully evidenced.

## Claim and Figure Evidence Ledger

For serious manuscripts, reports, grants, or scientific sections, maintain a compact claim/evidence ledger before finalizing. This can be a `.paper/claims.yml`, a table in project notes, or an equivalent source ledger.

Track:

- claim id and manuscript sentence or section
- supporting literature, dataset, code result, figure, table, or experiment artifact
- status: supported, draft, gap, or rejected
- risk language: what must be hedged, verified, or removed
- linked figure or table, if the claim is visualized

Also keep a figure inventory for important visuals: source file, panels, key numbers, supported claims, and caption role. Do not let a strong conclusion, novelty statement, mechanism claim, or limitation enter the final manuscript if its ledger status is `gap` or `rejected`.

## When to Use This Skill

This skill should be used when:
- Writing or revising any section of a scientific manuscript (abstract, introduction, methods, results, discussion)
- Writing or revising a formal research report or technical report
- Structuring a research paper using IMRAD or other standard formats
- Formatting citations and references in specific styles (APA, AMA, Vancouver, Chicago, IEEE)
- Planning how figures and tables support the document; use `scientific-visualization` or project analysis code to create or materially edit the visualization
...
Figures are evidence carriers, not decoration. Use visuals when they support a claim, clarify a difficult method, present data, show a workflow, or satisfy a venue requirement. Do not generate graphical abstracts, schematics, or illustrative images by default for every scientific document.

Prioritize:

- data-derived figures from the project's analysis code
- Mermaid/TikZ flowcharts for auditable workflows
- MATLAB/Origin/Python figures when numerical or experimental results drive the visual
- `scientific-schematics` or `generate-image` only when a generated visual materially improves communication and the toolchain is available

If a graphical abstract is required by the target venue or requested by the user, make it after the paper's evidence structure is stable, and record its role in the figure inventory.
...
**Abstract Composition**: Craft concise, standalone summaries that capture the paper's purpose, methods, results, and conclusions within the target venue's length and structure requirements.
...
- Include recent literature appropriate to the pace and conventions of the field
...
- Use each table or figure only when it communicates evidence more clearly than prose
...
- Prefer direct sentences, varying their length to fit the argument and improve clarity
...
1. Start from project evidence. When literature support is needed, use `research-literature-first`, then choose `paper-lookup` for targeted discovery or `literature-review` for broad or systematic synthesis. Add `citation-management` only for metadata, bibliography, or citation-style work; use `research-lookup` only for supplemental current web context.
...
Stage 1 should use only the evidence support the section needs:
1. Begin with project evidence and `research-literature-first` when scholarly support is required
2. Choose `paper-lookup` for targeted discovery or `literature-review` for broad or systematic synthesis
3. Add `citation-management` only when metadata, bibliography, or citation-style verification is required
4. Use `research-lookup` only for supplemental current web context, standards pages, or industry signals
5. Organize findings as bullet points in the outline, then convert prose sections to full paragraphs in Stage 2
...
When the user requests a LaTeX/PDF report, or when LaTeX is appropriate and no template is supplied, the bundled `scientific_report.sty` is an optional starting point. Do not convert a report to LaTeX merely because this asset exists; follow the requested file format and supplied template.
...
2. For each section below, follow the two-stage process:
   - **First**: Create outline with bullet points tied to literature, citation records, project evidence, and optional supplemental web context
...
- **scientific_report.sty**: Optional local style for a requested or appropriate LaTeX report
...
| Journal manuscripts | Use `venue-templates` skill |
| Conference papers | Use `venue-templates` skill |
| Research reports | Use the supplied or requested format; `scientific_report.sty` is optional for LaTeX |
| White papers | Use the supplied or requested format; `scientific_report.sty` is optional for LaTeX |
| Technical reports | Use the supplied or requested format; `scientific_report.sty` is optional for LaTeX |
| Grant reports | Follow the funder's or user's required format |
