...
def is_code_like(text: str, style: str) -> bool:
    ...
    return code_hits >= 2 or (
        len(text) > 120 and text.count("{") + text.count("}") + text.count("_") > 8
    )
...
def analyze(docx: Path) -> dict:
    ...
    omml_count = len(root.findall(".//m:oMath", NS)) + len(
    ...
    for idx, p in enumerate(paras):
        ...
        has_math = (
            p.find(".//m:oMath", NS) is not None
            or p.find(".//m:oMathPara", NS) is not None
        )
        if (
            not has_math
            and RAW_FORMULA_RE.search(text)
            and not is_code_like(text, style_id(p))
        ):
            raw_formula_paras.append({"paragraph_index": idx, "text": text[:220]})

    color_values = sorted(
        set(re.findall(r'w:color\s+w:val="([^"]+)"', document_xml + "\n" + styles_xml))
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check DOCX semantic issues common in contest papers."
    )
    parser.add_argument("docx", type=Path)
    parser.add_argument("--json", type=Path, help="Optional JSON report path.")
    parser.add_argument(
        "--no-strict", action="store_true", help="Always exit 0; still print findings."
    )
