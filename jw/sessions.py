...
Per-step pruning:
    ...
    causing unbounded growth (multi-GB sessions.db). JW never
...
def get_db_path() -> Path:
    ...
    Uses ``paths.DATA_DIR`` (~/.jw/ by default), then applies
...
# Production callers use ``JWConfig.checkpoint_keep_per_thread``.
...
def _resolve_keep_per_ns() -> int:
    """Resolve the retention count from JWConfig, with safe fallback."""
    try:
...
async def get_checkpointer() -> AsyncIterator[PruningCheckpointer]:
    ...
    retention count is read from ``JWConfig`` at context entry;
...
async def _load_checkpoint_messages(
    ...
    """
    # Pre-resolve the latest main JW checkpoint_id, then pin it
    # into the config so
...
async def list_threads(
    ...
    """List JW threads, most-recent first.
...
async def get_most_recent() -> str | None:
    """Return the most recent JW thread ID, or ``None``."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return None
...
async def thread_exists(thread_id: str) -> bool:
    """Return ``True`` if *thread_id* has at least one JW checkpoint."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return False
...
async def find_similar_threads(thread_id: str, limit: int = 5) -> list[str]:
    """Find JW thread IDs that start with *thread_id* (prefix match)."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return []
...
async def delete_thread(thread_id: str) -> bool:
    """Delete all JW checkpoints (and writes) for *thread_id*."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return False
...
async def get_thread_messages(thread_id: str) -> list:
    ...
    Only returns messages for JW threads.
    ...
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return []
        # Verify this thread belongs to JW before loading messages
        check = f"""
            SELECT 1 FROM checkpoints
            WHERE thread_id = ? AND {MAIN_THREAD_FILTER_SQL}
            LIMIT 1
...
# ---------------------------------------------------------------------------
# DB stats (read-only diagnostic for `jw sessions stats`)
# ---------------------------------------------------------------------------
...
async def db_stats(top_n: int = 5) -> dict[str, Any]:
    ...
    ``thread_count`` (int, JW threads), ``checkpoint_count``
    (int, JW rows), ``write_count`` (int, JW writes
    only — scoped via JOIN to ``checkpoints.metadata.agent_name`` so
    co-located non-jw agents are excluded), ``top_threads`` (list of
    ...
    try:
        async with aiosqlite.connect(str(db_path), timeout=30.0) as conn:
            ...
            if await _table_exists(conn, "writes"):
                # Scope writes to JW rows by joining against
...
def _api_workspace_dir() -> str:
    ...
    ``start_langgraph_dev`` injects ``JW_WORKSPACE_DIR`` and sets
    ...
    ws = os.environ.get("JW_WORKSPACE_DIR", "").strip()
    if ws:
...
def _publish_api_workspace_sidecar() -> None:
    ...
    The normal ``jw deploy`` launcher writes this sidecar before starting
    ...
    workspace = os.environ.get("JW_WORKSPACE_DIR", "").strip()
    if not workspace:
        return
    try:
        import psutil
...
async def _purge_internal_worker_threads() -> None:
    ...
    try:
        ...
        async with aiosqlite.connect(db_path, timeout=30.0) as conn:
            ...
            if await _table_exists(conn, "writes"):
                await conn.execute(
                    ...
                    DELETE FROM writes WHERE thread_id IN (
                        ...
                        WHERE json_extract(metadata, '$.graph_id') LIKE 'jwmemory-%'
                    )
                    """
                )
            cur = await conn.execute(
            ...
            if cur.rowcount:
                _logger.info(
                    "Purged %d leftover jwmemory-worker checkpoint row(s).",
                    cur.rowcount,
                )
    except Exception:
        _logger.warning(
            "jwmemory-worker residue purge failed (non-fatal).", exc_info=True
        )
