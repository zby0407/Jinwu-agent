...
def _supports_openrouter_anthropic_prompt_cache(provider: str, model_id: str) -> bool:
    """Return whether JW should declare OpenRouter Claude caching."""
...
def _apply_openrouter_anthropic_prompt_cache(
    ...
    """
    if _env_flag_disabled("JW_OPENROUTER_ANTHROPIC_PROMPT_CACHE"):
        return
...
def get_chat_model(
    ...
    **kwargs: Any,
    ...
    # OpenRouter → native ChatOpenRouter via init_chat_model.
        ...
        effort = os.environ.get("JW_REASONING_EFFORT", "").strip() or "high"
    ...
    if provider == "openai":
        _responses_api_setting = (
            os.environ.get("JW_USE_RESPONSES_API", "").strip().lower()
        )
