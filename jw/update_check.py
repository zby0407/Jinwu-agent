"""Background update check for JW.
...
PYPI_URL = "https://pypi.org/pypi/jw/json"
...
def _installed_version() -> str:
    """Return the currently installed JW version."""
    from importlib.metadata import version as _pkg_version

    return _pkg_version("jw-agent")
...
def get_latest_version() -> str | None:
    """Fetch the latest JW version from PyPI, with 24h caching."""
    # Try cache first
    try:
    ...
    try:
        ...
        req = urllib.request.Request(
            PYPI_URL,
            headers={"User-Agent": "JW update-check"},
        )
...
def is_update_available() -> tuple[bool, str | None]:
    """Check whether a newer version of JW is available on PyPI.
