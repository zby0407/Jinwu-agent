...
def _parser(skill_name: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Input CSV path; repeat for multiple sources.",
    )
    parser.add_argument("--mode", choices=["plan", "execute"], default="plan")
    parser.add_argument(
        "--approved",
        action="store_true",
        help="Confirm the planned business-data writes.",
    )
    parser.add_argument("--run-id")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--session-file")
    parser.add_argument(
        "--use-llm-semantics", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--include-strategy", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--run-tests", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument("--action", choices=["propose", "apply"], default="propose")
    parser.add_argument(
        "--split-proposal", help="JSON split proposal previously confirmed by the user."
    )
    return parser
...
def _planned_writes(skill_name: str, run_id: str) -> list[str]:
    values = {
        "ingest-align-solar-data": [
            "data/uploads/<dataset-id>/",
            "data/processed/uploads/<dataset-id>/",
        ],
        "clean-solar-data": [
            "data/processed/uploads/<dataset-id>/cleaned_v1.csv",
            "data/processed/uploads/<dataset-id>/quality_report.json",
        ],
        "engineer-solar-features": [
            "data/processed/uploads/<dataset-id>/engineered_features.csv",
            "data/processed/uploads/<dataset-id>/feature_registry.json",
        ],
        "prepare-experiment-handoff": [
            "data/processed/uploads/<dataset-id>/experiment_handoff.json",
            "data/processed/uploads/<dataset-id>/strategy_recommendation.json",
        ],
        "rebuild-solar-data-pipeline": ["data/processed/"],
        "run-solar-feature-workflow": [
            f"data/processed/skill_runs/{run_id}/run_manifest.json",
            "data/uploads/<dataset-id>/",
            "data/processed/uploads/<dataset-id>/",
        ],
    }
...
def run_skill_cli(skill_name: str, argv: list[str] | None = None) -> int:
    ...
    mutates = skill_name not in READ_ONLY_SKILLS and not (
        skill_name == "clean-solar-data" and args.action == "propose"
    )
    if args.mode == "plan":
        if skill_name == "run-solar-feature-workflow" and args.input:
            split_proposal = (
                json.loads(args.split_proposal) if args.split_proposal else None
            )
            ...
            payload = _envelope(
                ...
                run_id,
                stage="plan",
                planned_writes=planned if mutates else [],
            )
    ...
    session_path = (
        ...
        else ROOT / "data" / "processed" / "skill_runs" / run_id / "session.json"
    )
    ...
    try:
        if skill_name == "audit-solar-data-quality":
            if len(args.input) != 1:
                raise ValueError(
                    "audit-solar-data-quality requires exactly one --input"
                )
            result = audit_solar_data(args.input[0])
        elif skill_name == "ingest-align-solar-data":
            result = ingest_align_solar_data(
                args.input,
                session=session,
                use_llm_semantics=args.use_llm_semantics,
                split_proposal=split_proposal,
            ...
            if not current:
                raise ValueError(
                    "Provide --input or --session-file with a current dataset"
                )
            result = (
                propose_solar_cleaning(current, session=session)
                if args.action == "propose"
                else apply_solar_cleaning(session)
            )
            ...
            result = (
                audit if audit["status"] != "ok" else engineer_solar_features(session)
            )
            ...
            if args.input and not session.get_current_dataset_path():
                ...
                current = session.get_current_dataset_path()
                audit = (
                    audit_solar_data(current, session=session)
                    if current
                    else {"status": "failed"}
                )
                if audit.get("status") != "ok":
                    ...
                    result = (
                        ...
                        else prepare_experiment_handoff(
                            session, include_strategy=args.include_strategy
                        )
                    )
            else:
                result = prepare_experiment_handoff(
                    session, include_strategy=args.include_strategy
                )
            ...
            if preflight["status"] != "planned":
                ...
                result = run_solar_feature_workflow(
                    args.input,
                    session=session,
                    run_id=run_id,
                    resume=args.resume,
