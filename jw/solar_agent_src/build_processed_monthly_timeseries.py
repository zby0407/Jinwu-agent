...
def build_f107_monthly_median() -> pd.DataFrame:
    raw = pd.read_csv(
        RAW_DIR / "f107_daily_flux.csv", dtype={"date_utc": str}, low_memory=False
    )
    ...
    raw["missing_flag"] = (
        raw["missing_flag"].astype(str).str.lower().eq("true")
        | raw["f107_adjusted"].isna()
    )
    ...
    monthly = daily.groupby("date_month", as_index=False).agg(
        f107_monthly_median=("f107_adjusted_daily_mean", "median")
    )
    return monthly
...
def add_sunspot_smoothing_and_activity_flags(master: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in master[master["cycle_no"].notna()].groupby("cycle_no", sort=True):
        ...
        if pd.isna(smoothed_max) or smoothed_max <= 0:
            continue
        idx = group.index
        master.loc[idx, "is_peak_window_13m"] = (
            master.loc[idx, "sunspot_smoothed_13m"]
            .ge(0.85 * smoothed_max)
            .fillna(False)
        )
        master.loc[idx, "is_high_activity_phase"] = (
            master.loc[idx, "sunspot_smoothed_13m"]
            .ge(0.50 * smoothed_max)
            .fillna(False)
        )
    return master
...
def main() -> None:
    ...
    master = pd.DataFrame(
        {"date_month": pd.date_range(min_month, max_month, freq="MS")}
    )
    ...
    wso_small["polar_mean_abs"] = (
        wso_small["north_field_filtered_abs_mean"]
        + wso_small["south_field_filtered_abs_mean"]
    ) / 2
    wso_small = wso_small.drop(
        columns=["north_field_filtered_abs_mean", "south_field_filtered_abs_mean"]
    )
    ...
    master["sunspot_quality_flag"] = quality_flag_from_conditions(
        ...
        limited=~bool_series(master["has_uncertainty"])
        | ~bool_series(master["has_observation_count"]),
    )
    master["hemisphere_quality_flag"] = quality_flag_from_conditions(
        missing=master["north_sunspot_number"].isna()
        | master["south_sunspot_number"].isna(),
        provisional=bool_series(master["hemisphere_is_provisional"]),
        partial=bool_series(master["hemisphere_missing_value_flag"]),
        external_calibrated=bool_series(
            master["hemisphere_is_external_calibrated_observation"]
        ),
    )
    master["f107_quality_flag"] = quality_flag_from_conditions(
        missing=master["f107_monthly_mean"].isna(),
        partial=(master["f107_month_completeness"].fillna(0) < 0.95)
        | bool_series(master["f107_anomaly_flag"]),
    )
    master["polar_quality_flag"] = quality_flag_from_conditions(
        missing=master["polar_north"].isna() | master["polar_south"].isna(),
        partial=(master["wso_month_completeness"].fillna(0) < 1.0)
        | bool_series(master["polar_anomaly_flag"]),
    )
    ...
    in_goes_coverage = master["date_month"].between(
        GOES_COVERAGE_START, GOES_COVERAGE_END
    )
    if goes is None:
        ...
        for col in flare_count_cols:
            if col in master.columns:
                master.loc[in_goes_coverage, col] = master.loc[
                    in_goes_coverage, col
                ].fillna(0)
        for col in [
            "xray_peak_flux_sum_proxy",
            "xray_peak_flux_max_proxy",
        ...
        master.loc[
            master["date_month"].lt(GOES_COVERAGE_START), "flare_coverage_status"
        ] = "outside_coverage"
        master.loc[
            master["date_month"].gt(GOES_COVERAGE_END), "flare_coverage_status"
        ] = "outside_legacy_goes_xrs_report"
        master.loc[
            in_goes_coverage & master["flare_coverage_status"].isna(),
            "flare_coverage_status",
        ] = "observed_zero_event"
        master.loc[
            in_goes_coverage & master["flare_data_quality_flag"].isna(),
            "flare_data_quality_flag",
        ] = "observed_zero_event"
        master.loc[
            in_goes_coverage & master["flare_evidence_tier"].isna(),
            "flare_evidence_tier",
        ] = "auxiliary"
        master.loc[
            in_goes_coverage & master["flare_legacy_duration_warning"].isna(),
            "flare_legacy_duration_warning",
        ] = False

    coverage_sources = {
        "sunspot": master["sunspot_number"].notna(),
        "hemisphere": master["north_sunspot_number"].notna()
        & master["south_sunspot_number"].notna(),
    ...
    master["data_coverage_flag"] = [
        ...
        if count == len(coverage_sources)
        else "partial:"
        + "|".join(name for name, mask in coverage_sources.items() if mask.iloc[i])
    ...
    print(
        f"rows={len(final)} range={final['date_month'].min()}..{final['date_month'].max()}"
    )
