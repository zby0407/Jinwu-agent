"""Skill installation and management for JW.

This module provides functions for installing, listing, and uninstalling user skills.
Skills are installed to GLOBAL_SKILLS_DIR by default (~/.jw/skills/).
...
Usage:
    from jw.tools.skills_manager import install_skill, list_skills, uninstall_skill
...
def install_skill(
    ...
    Args:
        ...
        global_install: If True (default), install to GLOBAL_SKILLS_DIR
            (~/.jw/skills/). If False, install to the
    ...
    if _is_github_url(source):
        ...
        if not source_path.exists():
            ...
            # If not local and not a GitHub URL, try remote lookup in JWSkills
...
def _install_from_github(
    ...
    *record_as* overrides what gets written to the install manifest. By default
    ...
    with tempfile.TemporaryDirectory(prefix="jw-skill-") as tmp:
...
def list_skills(include_system: bool = False) -> list[SkillInfo]:
    ...
    # Tier 2: global skills (~/.jw/skills/)
...
def fetch_remote_skill_index(
    ...
    index: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="jw-browse-") as tmp:
        clone_dir = os.path.join(tmp, "repo")
        _clone_repo(repo, ref, clone_dir)
