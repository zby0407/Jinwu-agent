"""``jw deploy`` — start a standalone langgraph dev server.

Hosts the fully-equipped JW main agent (MCP + async sub-agents)
for consumption by external LangChain-compatible UIs (deep-agents-ui,
agent-chat-ui, LangSmith Studio) and SDK clients.

Differs from ``jw`` / ``jw serve``: no in-process CLI agent,
...
Mode dispatch happens via the ``JW_DEPLOY_MODE`` env var
...
and the agent build code (``agent.py:_get_default_agent``)
...
def deploy(
    ...
    """Deploy JW main agent as a standalone LangGraph dev server.
    ...
    if workdir:
        ...
        ws = os.getcwd()
    # Subprocess inherits this path via JW_WORKSPACE_DIR (set inside
    ...
    # 4. Pre-flight port check — refuse to start if a non-jw process is
    # holding the port. If an existing jw langgraph dev is already up,
    ...
    if _is_port_occupied(effective_port):
        if is_langgraph_dev_running(port=effective_port):
            ...
            console.print(
                "[dim]Stop the existing jw/serve session first, or use "
    ...
    console.print(
        Panel(
            ...
            title="[bold cyan]JW Deploy[/bold cyan]",
            border_style="cyan",
        )
    )
