...
JW scheduled tasks ARE langgraph crons targeting the ``scheduler``
...
Isolation is **process-level**, not data-level: JW's manager.py restarts
...
def list_schedules() -> list[Cron]:
    """Return only JW scheduled tasks.
