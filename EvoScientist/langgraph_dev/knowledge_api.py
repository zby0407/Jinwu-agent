...
The WebUI reads ``~/.evoscientist/knowledge.db`` through these routes.
...
try:
    ...
    def default_db_path() -> Path:  # type: ignore[no-redef]
        """Same rule as ``EvoScientist/paths.py`` (and store.py)."""

        override = os.getenv("EVOSCIENTIST_DATA_DIR")
        base = (
            Path(override).expanduser() if override else Path.home() / ".evoscientist"
        )
