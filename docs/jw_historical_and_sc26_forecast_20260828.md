...
极小期极区场模型使用 MWO–WSO 校准的南北极区孔径场绝对值平均作为前兆，以第 15—19 周作为初始训练窗口，逐周留出第 20—24 周，共 5 个测试折。模型同时与训练均值和持续性基线比较，并以活动周为单位进行 10,000 次 bootstrap。

该模型的样本外结果是当前历史预测中较有希望的一部分，但样本只有 5 个测试折；MWO 只覆盖 1 折，WSO 覆盖 4 折。因此它支持“可能存在方向性改善”，还不支持“已经获得稳定预测技能”。
...
图 3 是 H2 的 5 折回测。候选模型在五个留出折的点估计改善均为正，约为 **11.257、21.902、14.618、12.312、5.176**；但这组结果仍受小样本、相邻周期依赖和 MWO/WSO 制度差异限制。
...
本稿中的曲线由记录在案的 CSV/JSON 结果重新绘制，绘图脚本为 [`scripts/plot_jw_historical_sc26_forecast.py`](../scripts/plot_jw_historical_sc26_forecast.py)。相关历史预测数据和正式预测回执位于 `research/review/evals/runs/sc26_forecast_backtest_20260827/results/`；H2 极区场回测数据位于 `research/review/evals/runs/jw_solar_upgrade_20260828/` 对应实验运行目录。
