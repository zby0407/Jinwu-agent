...
诊断、异常行、PNG 图、JSON 摘要和独立完整性清单。生成结果位于忽略目录
...
- 服务器原始结果清单中的 46 项全部通过复核，5 条处理错误记录与
...
在 feature 分支完成后，使用仓库虚拟环境对加载、盘点、日/月聚合、特征分析、
服务器批处理以及 HMI 验证入口进行了独立复核：

- 相关测试共 50 项，结果为 `50 passed`；两条警告仅来自测试 FITS 中超过 8 字符的
  `HSOS_NUMBER` 关键字被 Astropy 写为 `HIERARCH` 卡，不影响处理断言。
- 从已归档 daily/monthly 输入重新运行特征分析，重新生成的 9 个核心 CSV 与已存结果
  逐文件一致，包括日/月特征、南北配对、年度、仪器年代、季节性、配对
  摘要以及日/月异常表。
- 5718 个支持文件中有 5713 个成功处理，5 个退化文件被完整记录，文件处理成功率约为
  99.91%。该结果支持文件路由、解码、聚合和产物复现的工程可靠性。
- 月覆盖并不连续：月观测日占自然日比例的中位数为 9.68%，447 条月记录中有 124 条
  仅含 1 个观测日（27.74%），月观测日数中位数为 3 天。因此低覆盖月必须保留覆盖
  标记，不能与高覆盖月等同解释。
- 当前产品尚未通过物理生产门槛：标准太阳 WCS 缺失，固定日面纬度孔径尚未实现，
  P0/P1 符号和 `CALIBRAT` 物理含义未由真实外部参考确认，本地 HMI 示例清单没有形成
  有效 N/S 配对。现有结果只能视为 `diagnostic_unvalidated` 工程代理。

复核结论分为三层：文件解码、日/月聚合和结果复现可靠；相同 `instrument_epoch` 与
`signal_unit` 内的相对变化和异常候选有条件可用；跨仪器长期趋势、真实磁极性、绝对
磁场强度、固定纬度极区场以及正式太阳周期前兆预测目前不可靠。

## 覆盖特征

- 日期范围：1987-12-29 至 2026-07-15。
- 范围内没有形成可处理 `NPL/SPL` 极区输出的年份：1988、2010–2013、
  2018–2019、2021。这不等于归档中完全没有 FITS；例如已核验的
  `L518ful180905015753.fit` 是 2018 年 `FUL` 全日面纵向观测，但没有南北极标识和
  太阳 WCS，不能由当前极区流程安全转换为 N/S 极区记录。
- **2018/2019/2021：存在 `FUL` 全日面 FITS，但没有可直接处理的 `NPL/SPL`
  极区专用文件。**
